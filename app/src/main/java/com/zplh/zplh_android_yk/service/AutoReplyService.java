    ...
    public void onAccessibilityEvent(final AccessibilityEvent event) {
        ...
        switch (eventType) {
            ...
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                ...
                if (!hasAction){
                    LogUtils.d("zzzzzzzzzzzz");
                    break;
                }
                itemNodeinfo = null;
                String className = event.getClassName().toString();
                if (className.equals("com.tencent.mm.ui.LauncherUI")) {
                    LogUtils.d("aaaaaaaaaaaaaaaaaaaa");
                    if (fill()) {
                        LogUtils.d("bbbbbbbbbbbbbb_");
                        send();
                    }else {
                        LogUtils.d("ccccccccccccccccccc");
                        if(itemNodeinfo != null){
                            LogUtils.d("ddddddddddddddd");
                            itemNodeinfo.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                            handler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    if (fill()) {
                                        LogUtils.d("eeeeeeeeeeeeee");
                                        send();
                                    }
                                    back2Home();
                                    release();
